...
renderTable({dataframe_filtered() %>% arrange(Priority_clusters)})
...
renderGauge({
  ...
  gauge(value, min = min, max = max, symbol = '', 
        gaugeSectors(success = c(min, step-0.5), warning = c(step, 2*step-0.5), 
                     danger = c(2*step, max)
  ))
})
```
...
renderGauge({
  ...
  gauge(value, min = min, max = max, symbol = '%', gaugeSectors(
    success = c(80, max), warning = c(40, 79), danger = c(min, 39)
  ))
})
```
