...
renderPlot({dataframe_filtered() %>% mutate(today = rep(Sys.Date(), length(Freshdays)))  %>% select(Product_ID, today, Expiration, Freshdays,Category, Priority_clusters) %>%  mutate(Expiration_date = as.Date(Expiration)) %>% gather(date,time, today, Expiration_date)  %>%
